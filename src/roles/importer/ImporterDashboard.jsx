...
const ImporterDashboard = () => {
  ...
  // Dashboard Stats - Only this importer's data
  const stats = [
  ...
  // Assignments Data - Importer's assignments with clearance progress
  const assignmentsData = [
    {
      id: 'ASN-001',
      shipmentId: '#458',
      agent: 'Swift Clearance Services',
      status: 'In Progress',
      progress: 70,
      daysInBond: 0,
      riskLevel: 'low',
      ...
      clearanceStage: 'Document Submission',
      documentsStatus: { total: 5, completed: 5, pending: 0 },
      priority: 'Normal'
    },
    {
      id: 'ASN-002',
      ...
      clearanceStage: 'Customs Hold',
      documentsStatus: { total: 5, completed: 3, pending: 2 },
      priority: 'High'
    },
    {
      id: 'ASN-003',
      shipmentId: '#462',
      agent: 'East Africa Customs Solutions',
      status: 'At Risk',
      progress: 60,
      daysInBond: 7,
      riskLevel: 'critical',
      ...
      clearanceStage: 'Duty Assessment',
      documentsStatus: { total: 5, completed: 4, pending: 1 },
      priority: 'Urgent'
    },
    {
      id: 'ASN-004',
      shipmentId: '#460',
      agent: 'Mombasa Port Logistics',
      status: 'Completed',
      progress: 100,
      daysInBond: 0,
      riskLevel: 'low',
    ...
    },
    {
      ...
      clearanceStage: 'Vessel Departure',
      documentsStatus: { total: 5, completed: 2, pending: 3 },
      priority: 'Normal'
    }
  ];

  // Shipments Data - Sort by created date (newest first)
  const allShipments = [
    {
      id: '#458',
      ...
      createdDate: new Date(2026, 5, 15),
      items: 450,
      weight: '12.5 tons',
    ...
    },
    {
      ...
      createdDate: new Date(2026, 7, 20),
      items: 200,
      weight: '6.8 tons',
      container: 'DE-782341',
  ...
  const getFilteredAssignments = () => {
    ...
    if (assignmentSearch) {
      ...
      filtered = filtered.filter(a => 
        a.shipmentId.toLowerCase().includes(search) ||
        a.agent.toLowerCase().includes(search) ||
  ...
  // Documentation Status - Importer's documents
  const documents = [
  ...
  // Container Tracking - Importer's containers
  const containers = [
    {
      id: 'MSKU-458921',
      status: 'At Sea',
  ...
  const getRiskBadge = (riskLevel) => {
    const riskMap = {
      'low': { bg: colors.success + '20', color: colors.success, label: 'Low', icon: <CheckCircle className="w-3 h-3" /> },
      'medium': { bg: colors.warning + '20', color: colors.warning, label: 'Medium', icon: <AlertCircle className="w-3 h-3" /> },
      'high': { bg: colors.danger + '20', color: colors.danger, label: 'High', icon: <AlertTriangle className="w-3 h-3" /> },
  ...
  const getAssignmentStatusBadge = (status) => {
    const statusMap = {
      'In Progress': { bg: colors.info + '20', color: colors.info, icon: <Clock className="w-3 h-3" /> },
      'Pending Assignment': { bg: colors.warning + '20', color: colors.warning, icon: <AlertCircle className="w-3 h-3" /> },
      'At Risk': { bg: colors.danger + '20', color: colors.danger, icon: <AlertTriangle className="w-3 h-3" /> },
      'Completed': { bg: colors.success + '20', color: colors.success, icon: <CheckCircle className="w-3 h-3" /> }
    };
    return statusMap[status] || statusMap['Pending Assignment'];
  };
  ...
  // Format date for assignments table
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`pl-8 pr-3 py-1.5 rounded-lg border text-xs focus:outline-none focus:ring-2 transition-all duration-200 ${
                  ...
                  <select
                    ...
                    className={`px-3 py-1.5 rounded-lg border text-xs focus:outline-none focus:ring-2 transition-all duration-200 ${
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className={`border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                      <th className={`text-left py-3 px-3 font-medium text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                        Shipment
                      </th>
                      <th className={`text-left py-3 px-3 font-medium text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                        Agent
                  ...
                  <tbody>
                    {filteredAssignments.length > 0 ? (
                      filteredAssignments.map((assignment) => {
                        ...
                        const progressColor = assignment.progress > 70 ? colors.success : 
                                             assignment.progress > 40 ? colors.warning : colors.danger;

                        return (
                          <tr key={assignment.id} className={`border-b ${isDark ? 'border-gray-700' : 'border-gray-100'} hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors`}>
                            ...
                            <td className="py-3 px-3">
                              <div className="flex items-center gap-2">
                                {assignment.agent !== 'Not Assigned' ? (
                                  ...
                                  <span className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'} italic`}>
                                    Not Assigned
                                  </span>
                                )}
                              </div>
                            ...
                            <td className="py-3 px-3">
                              <span className={`text-sm font-medium ${
                                assignment.daysInBond > 7 ? 'text-red-500' : 
                                assignment.daysInBond > 5 ? 'text-yellow-500' : 
                                'text-green-500'
                              }`}>
                                {assignment.daysInBond} days
                              </span>
                            </td>
                            <td className="py-3 px-3">
                              <span className={`text-xs px-2 py-1 rounded-full flex items-center gap-1`}
                            ...
                            <td className="py-3 px-3">
                              <span className={`text-sm font-medium ${
                                assignment.estimatedFine > 0 ? 'text-red-500' : 'text-green-500'
                              }`}>
                                Ugx {assignment.estimatedFine.toLocaleString()}
                              </span>
                            </td>
                            <td className="py-3 px-3">
                              <div className="flex items-center gap-1">
                                <button
                                  onClick={() => navigate(`/importer/assignment/${assignment.shipmentId.replace('#', '')}`)}
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    ...
                    >
                      ...
                      <span className="whitespace-nowrap max-w-[80px] truncate">
                          ...
                          <div className={`border-t pt-3 ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                            ...
                            <div className="flex flex-col gap-2">
                              ...
                              <button
                                ...
                                className="w-full px-3 py-1.5 rounded text-xs font-medium text-white transition-all duration-200 hover:opacity-90"
