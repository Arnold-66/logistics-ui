// roles/importer/ImporterFreightBookings.jsx
...
import {
  ...
  Users, Building, Phone, Mail, User, BarChart3, Activity, Layers, Truck,
  Anchor, Globe, Flag, BookOpen, FileText, ArrowRight, Home, MapPin
} from 'lucide-react';
import { ThemeContext } from '../../context/themeContext';
import { useAuth } from '../../context/authContext';
import { useNavigate } from 'react-router-dom';

const ImporterFreightBookings = () => {
  ...
  const [bookings, setBookings] = useState([
    {
      ...
      documents: [
        { id: 1, name: 'Commercial Invoice', type: 'invoice', status: 'completed', date: '2026-07-22', size: '2.4 MB', documentNumber: 'INV-2026-001' },
        { id: 2, name: 'Bill of Lading', type: 'bl', status: 'pending', date: '2026-07-25', size: '1.8 MB', documentNumber: 'BOL-2026-001' },
        { id: 3, name: 'Packing List', type: 'list', status: 'completed', date: '2026-07-23', size: '1.2 MB', documentNumber: 'PL-2026-001' }
      ]
    },
    {
      id: 'FRT-2026-002',
      ...
      blNo: 'BL-2026-002',
      exporter: 'East Africa Trading Co',
      exporterContact: 'Peter Habimana',
      exporterPhone: '+256 700 234567',
      exporterEmail: 'peter@eastafrica.com',
      exporterAddress: 'Kampala, Uganda',
      consignee: 'Rwanda Importers Ltd',
      consigneeContact: 'Marie Uwimana',
      consigneePhone: '+250 788 123456',
      consigneeEmail: 'marie@rwandaimporters.com',
      consigneeAddress: 'Kigali, Rwanda',
      forwardingAgent: 'Rwanda Logistics',
      notifyParty: 'Kigali Shipping Agency',
      ...
      pointOfOrigin: 'Kampala',
      documents: [
        { id: 1, name: 'Commercial Invoice', type: 'invoice', status: 'pending', date: '2026-07-30', size: '1.6 MB', documentNumber: 'INV-2026-002' },
        { id: 2, name: 'COC Certificate', type: 'certificate', status: 'pending', date: '2026-08-01', size: '1.0 MB', documentNumber: 'COC-2026-002' }
      ]
    },
    {
      id: 'FRT-2026-003',
      ...
      blNo: 'BL-2026-003',
      exporter: 'Global Importers Inc',
      exporterContact: 'Sarah Kamau',
      exporterPhone: '+254 722 123456',
      exporterEmail: 'sarah@globalimporters.com',
      exporterAddress: 'Nairobi, Kenya',
      consignee: 'Nairobi Distributors',
      consigneeContact: 'Michael Ochieng',
      consigneePhone: '+254 722 789012',
      consigneeEmail: 'michael@nairobidistributors.com',
      consigneeAddress: 'Nairobi, Kenya',
      forwardingAgent: 'Kenya Logistics',
      notifyParty: 'Nairobi Shipping Agency',
      ...
      countryOfOrigin: 'Uganda',
      preCarriageBy: 'Truck',
      placeOfReceipt: 'Entebbe Warehouse',
      loadingPier: 'Main Terminal',
      originalsReleasedAt: 'Nairobi Office',
      countryFlag: 'Kenya',
      pointOfOrigin: 'Entebbe',
      documents: [
        { id: 1, name: 'Commercial Invoice', type: 'invoice', status: 'completed', date: '2026-07-12', size: '1.8 MB', documentNumber: 'INV-2026-003' },
        { id: 2, name: 'Proof of Delivery', type: 'details', status: 'completed', date: '2026-08-05', size: '2.2 MB', documentNumber: 'POD-2026-003' },
        { id: 3, name: 'Freight Invoice', type: 'invoice', status: 'completed', date: '2026-07-15', size: '3.1 MB', documentNumber: 'FI-2026-003' }
      ]
    },
    {
      id: 'FRT-2026-004',
      ...
      consigneeContact: 'Robert Ssali',
      consigneePhone: '+256 700 123456',
      consigneeEmail: 'robert@ugandamanufacturers.com',
      consigneeAddress: 'Kampala, Uganda',
      forwardingAgent: 'Uganda Logistics',
      notifyParty: 'Kampala Shipping Agency',
      vessel: 'MV Pacific Voyager',
      vesselSCAC: 'PAC',
      voyage: 'PV-2026-045',
      portOfLoading: 'Kampala, Uganda',
      ...
      pointOfOrigin: 'Kampala',
      documents: [
        { id: 1, name: 'Commercial Invoice', type: 'invoice', status: 'completed', date: '2026-08-30', size: '2.6 MB', documentNumber: 'INV-2026-004' },
        { id: 2, name: 'Bill of Lading', type: 'bl', status: 'completed', date: '2026-09-02', size: '1.9 MB', documentNumber: 'BOL-2026-004' },
        { id: 3, name: 'Customs Declaration', type: 'certificate', status: 'pending', date: '2026-09-05', size: '3.2 MB', documentNumber: 'CD-2026-004' },
        { id: 4, name: 'Packing List', type: 'list', status: 'completed', date: '2026-08-29', size: '1.1 MB', documentNumber: 'PL-2026-004' }
      ]
    }
  ]);
  ...
  const handleViewDocuments = (booking) => {
    navigate(`/importer/freight-booking/${booking.id}/documents`, { 
      state: { 
  ...
  const filteredBookings = bookings.filter(booking => {
    const matchesSearch = 
      ...
      booking.exporter.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: isDark ? '#1a1a2e' : '#f8fafc' }}>
        <div className="text-center">
          ...
          <p className={`mt-4 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Loading freight bookings...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
              View all freight bookings made by exporters
            </p>
            {user && (
              <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'} mt-1`}>
                <Building className="w-3 h-3 inline mr-1" />
                {user.companyName || 'Your Company'} • Importer View
              </p>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
          <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white shadow-sm'}`}>
            ...
            <p className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>{bookings.length}</p>
          </div>
          <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white shadow-sm'}`}>
            <div className="flex items-center gap-2">
            ...
            <p className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {bookings.filter(b => b.status === 'Pending Approval' || b.status === 'Pending Documentation').length}
            </p>
          </div>
          <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white shadow-sm'}`}>
            <div className="flex items-center gap-2">
            ...
            <p className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {bookings.filter(b => b.status === 'In Transit' || b.status === 'In Customs').length}
            </p>
          </div>
          <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white shadow-sm'}`}>
            <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search by booking no, exporter, consignee, vessel..."
              value={searchTerm}
                ...
                return (
                  <div key={booking.id} className="p-4">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                              ...
                              Exporter: {booking.exporter} → Consignee: {booking.consignee}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => navigate(`/importer/freight-booking/${booking.id}`)}
                    ...
                    {isExpanded && (
                      <div className="mt-3 pt-3 border-t space-y-3" style={{ borderColor: isDark ? '#374151' : '#e5e7eb' }}>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          <div>
                            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Exporter</p>
                            <p className={`text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{booking.exporter}</p>
                            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{booking.exporterEmail}</p>
                            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{booking.exporterPhone}</p>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Pier: {booking.loadingPier}</p>
                          </div>
                          <div>
                            ...
                            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Delivery: {booking.placeOfDelivery}</p>
                          </div>
                        </div>
...
export default ImporterFreightBookings;